...
{
    return this->operator=(std::u16string_view(rhs));
}
...
{
    return this->ToCharArray(0, _str.length());
}
...
std::vector<String> String::Split(std::initializer_list<char16_t> separator, int32_t count, StringSplitOptions options) const
{
    ...
    while (true)
    {
        const auto currentIndex = this->IndexOfAny(separator, prevIndex, count);
        if (currentIndex == -1)
        {
...
{
    return this->IndexOf(c, startIndex, _str.length() - startIndex);
}
...
{
    return this->IndexOf(value, startIndex, _str.length() - startIndex);
}
...
{
    return this->IndexOfAny(anyOf, startIndex, _str.length() - startIndex);
}
...
{
    return this->LastIndexOf(value, static_cast<int32_t>(_str.length()) - 1);
}
...
{
    return this->LastIndexOfAny(anyOf, static_cast<int32_t>(_str.length()) - 1);
}
